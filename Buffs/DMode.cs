...
namespace DMode.Buffs
{
	public class DMode : ModBuff
	{
		public override void SetStaticDefaults()
        {
            ...
            if (Main.expertMode) { description = "Prepare to die, Terrarian!"; }
            else if (!Main.expertMode) { description = "Unlimited Power!"; }
        ...
        {
            player.GetAttackSpeed<T>() += modifier;
        }
        ...
        public override void Update(Player player, ref int buffInDexterity)
	    {
            ...
            AddDamageModifier<MeleeDamageClass>(player, Strength * 0.008f * DestinyFactor);
            AddDamageModifier<MagicDamageClass>(player, Mind * 0.008f * DestinyFactor);
            AddDamageModifier<RangedDamageClass>(player, Dexterity * 0.008f * DestinyFactor);
            AddDamageModifier<ThrowingDamageClass>(player, (Strength + Dexterity) * 0.004f * DestinyFactor);
            ...
            AddKnockbackModifier<SummonDamageClass>(player, (Mind + Spirit) * 0.0075f * MinionBonusFactor * DestinyFactor);
            ...
            int BonusDefense = (int) (StrengthBonusDefense + MindBonusDefense + DexterityBonusDefense + SpiritBonusDefense);
